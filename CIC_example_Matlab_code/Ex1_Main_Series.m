...
%% Eq (21) and setup in p. 40 LB of KS(2013)
%Series system
H = @(x) min([  3 + (x(:,1) - x(:,2)).^2./10 - (x(:,1) + x(:,2))./sqrt(2) ... , 
                3 + (x(:,1) - x(:,2)).^2./10 + (x(:,1) + x(:,2))./sqrt(2) ... , 
                x(:,1) - x(:,2) + 7./sqrt(2) ... , 
                x(:,2) - x(:,1) + 7./sqrt(2)            ], [], 2) <= 0;
    %ezcontourf(@(x,y) H([x, y]), [-7 7], [-7 7], 400); colormap(gray)
...
grid_size = 400; %60; %200;
figure
ezcontourf(@(x,y) -pdf(GM,[x y]), [-7 7], [-7 7], grid_size)  %ezsurfc % ezsurf(@(x,y) -pdf(GM,[x y]), [-4 4], [0 5])
colormap(bone)%gray
% hold on
...
if (ifsave == 1)
    ...
    saveFileName = horzcat('Ex1_Series_n_exp_',num2str(n_exp),'_',datetime_str,'.mat');
    save(saveFileName);
end
