...
function [blackout,demand_lost] = sim_case39_fn2(branches,useVerbose)
    C = psconstants;        
    ...
    for i = 1:length(branches) 
        event(i+1,[C.ev.time C.ev.type]) = [3 C.ev.trip_branch];
        event(i+1,C.ev.branch_loc) = branches(i);
    end
    
    % set the end time
    event(length(branches)+2,[C.ev.time C.ev.type]) = [t_max C.ev.finish];

    %% run the simulation
    %% Create a filename which is unique to this MATLAB process so that we can run w/ Python multiprocessing
    fname = 'sim_case39_'+feature('getpid')
    [outputs,ps] = simgrid(ps,event,fname,opt);
    ...
    blackout = ~outputs.success;
    demand_lost = outputs.demand_lost;
